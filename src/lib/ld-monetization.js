...
export function validateCreatorEligibility(followerCount, minimumFollowers = 1000) {
  const count = Number(followerCount || 0);
  return Number.isFinite(count) && count >= Number(minimumFollowers || 0);
}

export function isValidOrangeMoneyNumber(value) {
...
export function generateUSSD(orangeMoneyNumber, amountLD) {
  const cleaned = String(orangeMoneyNumber || '').trim().replace(/\s+/g, '');
  const value = Number(amountLD || 0);
  const normalizedNumber = cleaned.startsWith('+231') ? cleaned.replace(/^\+231/, '231') : cleaned;
  const ussd = `*144*2*1*1*${normalizedNumber}*${Math.max(0, Math.round(value))}#`;
  return ussd;
}
