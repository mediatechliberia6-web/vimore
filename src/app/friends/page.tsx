          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredUsers.length > 0 ? filteredUsers.map((user, i) => {
              ...
              return (
                <div 
                  key={user.username} 
                  className="group relative bg-[#0A0A0A] border border-white/5 rounded-[2rem] p-5 transition-all hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5 animate-in fade-in slide-in-from-bottom-2 duration-500"
                  style={{ animationDelay: `${i * 50}ms` }}
                >
                  {/* Neon Diffusion Shadow */}
                  ...
                  <div className="relative flex items-center justify-between gap-4">
                    <Link href={`/profile/${user.username}`} className="flex items-center gap-4 flex-1 min-w-0">
                      <div className="relative shrink-0">
                        <Avatar className="h-16 w-16 border-2 border-white/5 transition-transform group-hover:scale-105 group-hover:border-primary/50">
