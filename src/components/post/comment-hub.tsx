...
export function ReelCommentHub() {
  const { activeReelCommentPostId, closeReelCommentHub, activeComments, addComment, addReply, triggerHaptic } = usePosts();
  const { t } = useTranslation();
  ...
  const handleSend = async () => {
    if (!text.trim() || !activeReelCommentPostId) return;
    triggerHaptic(20);
    const currentText = text;
    setText("");
    try {
      if (replyingTo) {
        await addReply(activeReelCommentPostId, replyingTo.$id, currentText);
      } else {
        await addComment(activeReelCommentPostId, currentText);
      }
      setReplyingTo(null);
    } catch {
      setText(currentText);
  ...
  if (!activeReelCommentPostId) return null;

  return (
    <Sheet open={!!activeReelCommentPostId} onOpenChange={(open) => !open && closeReelCommentHub()}>
      <SheetContent
        side="bottom"
        className="rounded-t-[3rem] p-0 border-primary/10 bg-black/90 backdrop-blur-3xl h-[55vh] flex flex-col transition-all duration-500 overflow-hidden z-[600]"
      >
        <div className="mx-auto w-12 h-1.5 bg-white/20 rounded-full mt-4 mb-2 shrink-0" />

        <SheetHeader className="px-6 py-3 border-b border-white/10 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <SheetTitle className="text-lg font-black italic uppercase tracking-tighter text-white">{t('comm_pulse')}</SheetTitle>
              <div className="bg-primary/20 px-3 py-1 rounded-full flex items-center gap-2 border border-primary/20">
            ...
            <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 text-white/60 hover:text-white hover:bg-white/10" onClick={closeReelCommentHub}>
              <X className="h-5 w-5" />
            </Button>
          </div>
        </SheetHeader>
            ...
            {topLevelComments.length > 0 ? (
              topLevelComments.map(comment => (
                <CommentNode
                  key={comment.$id}
                  comment={comment}
                  postId={activeReelCommentPostId}
              ...
              <div className="py-16 text-center space-y-4 opacity-60">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-black italic uppercase tracking-widest text-white">{t('comm_quiet')}</p>
                  <p className="text-[10px] font-medium uppercase text-white/50">{t('comm_first')}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 pb-8 bg-gradient-to-t from-black via-black/95 to-transparent">
          <div className="max-w-3xl mx-auto space-y-2">
            {replyingTo && (
              <div className="flex items-center justify-between bg-primary/20 px-4 py-2 rounded-xl animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="relative">
              <Input
                ...
                className="h-12 pl-5 pr-12 bg-white/10 border-white/20 text-white placeholder:text-white/40 rounded-2xl focus-visible:ring-primary/40 text-sm font-medium"
                value={text}
              ...
              <Button
                ...
                className={cn(
                  ...
                  text.trim() ? "bg-primary text-white shadow-lg" : "bg-white/5 text-white/20 opacity-40"
                )}
                disabled={!text.trim()}
                onClick={handleSend}
              >
