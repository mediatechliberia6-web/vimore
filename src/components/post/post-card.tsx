          ...
          {isHiddenByLock ? (
            <div className="relative min-h-[380px] w-full rounded-[2.5rem] overflow-hidden flex flex-col items-center justify-center p-8 bg-secondary/20">
              ...
              <div className="relative z-10 flex flex-col items-center text-center space-y-5">
                ...
                <Button className="bg-cyan-500 hover:bg-cyan-600 text-white rounded-2xl h-14 px-10 font-black italic uppercase tracking-widest text-sm shadow-xl shadow-cyan-500/20 active:scale-95 gap-3" onClick={async () => { setIsUnlocking(true); try { await unlockPost($id, unlockPrice!); } catch (e: any) { toast({ variant: "destructive", title: "Unlock Failed", description: e.message }); } finally { setIsUnlocking(false); } }} disabled={isUnlocking}>{isUnlocking ? <Loader2 className="h-5 w-5 animate-spin" /> : <Zap className="h-5 w-5 fill-current" />}{t('post_unlock_for')} {unlockPrice} Credits</Button>
              </div>
            </div>
          ) : (
            <>
