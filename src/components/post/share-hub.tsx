      ...
      <SheetContent side="bottom" className="rounded-t-[3rem] p-0 border-primary/10 bg-white/80 dark:bg-[#050505]/80 backdrop-blur-2xl h-[85vh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-12 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Button 
              variant="outline" 
              className="h-24 rounded-[2rem] border-green-500/10 bg-white dark:bg-card hover:bg-green-500/5 flex flex-col items-center justify-center gap-2 transition-all group"
              onClick={handleDownload}
              disabled={isDownloading}
            >
              <div className="p-3 bg-green-500/10 rounded-2xl group-hover:scale-110 transition-transform">
                {isDownloading ? <Loader2 className="h-6 w-6 text-green-500 animate-spin" /> : <Download className="h-6 w-6 text-green-500" />}
              </div>
            ...
            <Button 
              variant="outline" 
              className="h-24 rounded-[2rem] border-border bg-white dark:bg-card hover:bg-secondary/50 flex flex-col items-center justify-center gap-2 transition-all group"
          ...
          <div className="space-y-4">
            ...
            <div className="relative group">
              ...
              <Input 
                ...
                className="h-12 bg-secondary/30 border-none rounded-2xl pl-11 focus-visible:ring-primary/20"
                value={searchQuery}
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide px-1">
              {filteredConnections.map((user) => (
                <button 
                  key={user.username}
                  onClick={() => handleDirectShare(user.username)}
                  className="flex flex-col items-center gap-2 shrink-0 group min-w-[80px]"
                >
                  <div className="relative">
                    <div className={cn(
                      "absolute -inset-1 rounded-full blur-sm transition-all duration-500",
                      sharingTo === user.username ? "bg-primary animate-pulse opacity-100" : "bg-transparent opacity-0 group-hover:bg-primary/20 group-hover:opacity-100"
                    )} />
                    <Avatar className="h-14 w-14 border-2 border-white dark:border-card relative shadow-xl transition-transform group-active:scale-90">
                      <AvatarImage src={user.avatar} />
                      <AvatarFallback>{user.name[0]}</AvatarFallback>
                    </Avatar>
                    {sharingTo === user.username && (
                      <div className="absolute inset-0 flex items-center justify-center bg-primary/40 rounded-full z-10">
