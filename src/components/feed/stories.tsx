...
const mockStories = [
  { id: 1, name: "Your Story", avatar: "https://picsum.photos/seed/me/100/100", image: "https://picsum.photos/seed/s1/200/300", isMe: true },
        ...
        <div className="flex w-max space-x-2.5 p-1 pb-4">
          {mockStories.map((story) => (
            <div key={story.id} className="relative w-28 h-48 rounded-2xl overflow-hidden cursor-pointer group shadow-sm hover:shadow-md transition-shadow">
              ...
              {story.isMe ? (
                <div className="absolute bottom-0 w-full h-1/3 bg-white flex flex-col items-center justify-center p-2 pt-4">
                  <div className="absolute -top-4 w-8 h-8 bg-primary border-4 border-white rounded-full flex items-center justify-center text-white shadow-lg">
                    <Plus className="w-5 h-5" />
                  </div>
                  <span className="text-[10px] font-bold text-foreground">Create Story</span>
                </div>
              ) : (
                <>
