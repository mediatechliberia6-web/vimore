...
export function StoryViewer() {
  ...
  const handleTap = (e: React.MouseEvent | React.TouchEvent) => {
    // If long press was detected, don't trigger tap
    if (isPaused) return;
  ...
  }, [activeStory, segmentIndex, isPaused, nextSegment]);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[500px] h-full sm:h-[90vh] sm:rounded-2xl overflow-hidden bg-zinc-900 shadow-2xl flex flex-col">
            ...
            <div key={i} className="h-1 flex-1 bg-white/20 rounded-full overflow-hidden">
              <div 
                className={cn(
                  ...
                  i < segmentIndex ? "w-full" : i === segmentIndex && !isPaused ? "w-full" : "w-0"
                )}
              />
            </div>
          ))}
        ...
        {/* Interaction Bar (Phase 3 will expand this) */}
        <div className={cn(
        ...
        <div className="hidden sm:block">
          <Button 
            variant="ghost" 
            size="icon" 
            className="absolute -left-16 top-1/2 -translate-y-1/2 text-white bg-white/5 hover:bg-white/10 rounded-full h-12 w-12 border border-white/10"
            onClick={prevSegment}
          ...
          <Button 
            variant="ghost" 
            size="icon" 
            className="absolute -right-16 top-1/2 -translate-y-1/2 text-white bg-white/5 hover:bg-white/10 rounded-full h-12 w-12 border border-white/10"
            onClick={nextSegment}
          >
