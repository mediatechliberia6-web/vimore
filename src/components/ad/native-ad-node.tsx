...
export function NativeAdNode({ type, id, isActive }: NativeAdNodeProps) {
  ...
  // Adsterra Handshake: Injecting the node script
  useEffect(() => {
    if (typeof window === "undefined" || !containerRef.current) return;

    // We check if the script is already present to avoid multiple heavy loads
    // However, for native banners, we often need to re-run or use unique containers.
    // For this prototype, we'll append the script to the specific instance container.
    const script = document.createElement("script");
    script.src = "https://pl28803356.effectivegatecpm.com/da434b4b9d70fa28431080d1f00b7b40/invoke.js";
    script.async = true;
    script.setAttribute("data-cfasync", "false");
    
    // Cleanup existing content if any
  ...
  const handleLearnMore = () => {
    triggerHaptic(20);
    // Use the existing AdPortal logic to show the ad full screen for 10s
    // We pass an empty task because we just want to show the ad
      ...
      <div className="relative h-[100dvh] w-full flex items-center justify-center bg-black overflow-hidden group select-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-zinc-900 to-black flex items-center justify-center">
          ...
          <div className="relative z-10 w-full px-8 flex flex-col items-center gap-8">
            <div id="container-da434b4b9d70fa28431080d1f00b7b40" ref={containerRef} className="w-full min-h-[250px] flex items-center justify-center rounded-3xl overflow-hidden bg-white/5 border border-white/10" />
  ...
  return (
    <Card className="border-none shadow-sm overflow-hidden bg-white dark:bg-card mb-4 ring-1 ring-black/5 dark:ring-white/5 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <CardContent className="px-3 pb-3 space-y-4">
        <div className="relative group rounded-2xl overflow-hidden bg-secondary/20 min-h-[200px] flex items-center justify-center border border-primary/5">
          <div id="container-da434b4b9d70fa28431080d1f00b7b40" ref={containerRef} className="w-full flex justify-center" />
